...
const Messages:React.FC<{hostUserId:number}>=({hostUserId})=> {
  ...
  return (
    <div className="grid grid-cols-[360px_auto] max-md:grid-cols-[300px_auto] max-sm:block max-sm:relative overflow-hidden h-full w-full mr-10 max-sm:mr-0 mt-5 sm:pr-[20px] transition-all duration-[0.5s]">
      <div className={`bg-white grid grid-rows-[90px_auto] sm:px-6 max-sm:px-2 border-r-[#28303033] border-r border-solid overflow-hidden max-sm:absolute max-sm:w-full max-sm:h-full ${showChat?"max-sm:hidden":""}`}>
      ...
      <div className={`h-full overflow-hidden max-sm:absolute ${!showChat?"max-sm:hidden":""} max-sm:w-full`}>
